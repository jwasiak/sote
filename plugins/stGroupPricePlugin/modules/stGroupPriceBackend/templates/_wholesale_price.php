...
<?php foreach ($groups as $group): $getter_netto = 'getWholesale'.ucfirst($group).'Netto'; $getter_brutto = 'getWholesale'.ucfirst($group).'Brutto'; $getter_currency = 'getCurrencyWholesale'.ucfirst($group) ?>
            <td><?php echo input_tag ('group_price[wholesale]['.$group.'][netto]', st_price_format($group_price->$getter_netto()), array('size' => 7)) ?></td>
            <td><?php echo input_tag ('group_price[wholesale]['.$group.'][brutto]', st_price_format($group_price->$getter_brutto()), array('size' => 7)) ?></td>
<?php endforeach ?>
        </tr>
    </tbody>
</table>
