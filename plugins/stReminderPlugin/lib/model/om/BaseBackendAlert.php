...
/**
 * Base class that represents a row from the 'st_backend_alert' table.
 *
 * 
 *
...
abstract class BaseBackendAlert extends BaseObject  implements Persistent {
	...
	/**
	 * Collection to store aggregation of collBackendAlertI18ns.
	 * @var        array
	 */
	protected $collBackendAlertI18ns;

	/**
	 * The criteria used to select the current contents of collBackendAlertI18ns.
	 * @var        Criteria
	 */
  ...
  {
    try {
      if ($this->getDispatcher()->getListeners('BackendAlert.preHydrate')) {
          $event = $this->getDispatcher()->notify(new sfEvent($this, 'BackendAlert.preHydrate', array('resultset' => $rs, 'startcol' => $startcol)));
      ...
      $this->setNew(false);
      if ($this->getDispatcher()->getListeners('BackendAlert.postHydrate')) {
          $event = $this->getDispatcher()->notify(new sfEvent($this, 'BackendAlert.postHydrate', array('resultset' => $rs, 'startcol' => $startcol + 7)));
          return $event['startcol'];
      }

      // FIXME - using NUM_COLUMNS may be clearer.
      return $startcol + 7; // 7 = BackendAlertPeer::NUM_COLUMNS - BackendAlertPeer::NUM_LAZY_LOAD_COLUMNS).
  ...
  public function delete($con = null)
  {
    ...
    if ($this->getDispatcher()->getListeners('BackendAlert.preDelete')) {
        $this->getDispatcher()->notify(new sfEvent($this, 'BackendAlert.preDelete', array('con' => $con)));
    }

    if (sfMixer::hasCallables('BaseBackendAlert:delete:pre'))
    {
    ...
    if ($this->getDispatcher()->getListeners('BackendAlert.postDelete')) {
        $this->getDispatcher()->notify(new sfEvent($this, 'BackendAlert.postDelete', array('con' => $con)));
    }

    if (sfMixer::hasCallables('BaseBackendAlert:delete:post'))
    {
  ...
  public function save($con = null)
  {
    ...
    if (!$this->alreadyInSave) {
      if ($this->getDispatcher()->getListeners('BackendAlert.preSave')) {
    ...
    try {
      ...
      if (!$this->alreadyInSave) {
        if ($this->getDispatcher()->getListeners('BackendAlert.postSave')) {
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of BackendAlert (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     BackendAlert Clone of current object.
	 * @throws     PropelException
	 */
	...
	/**
	 * Temporary storage of collBackendAlertI18ns to save a possible db hit in
	 ...
	 * Otherwise if this BackendAlert has previously
	 * been saved, it will retrieve related BackendAlertI18ns from storage.
	 * If this BackendAlert is new, it will return
	...
	/**
	 * Returns the number of related BackendAlertI18ns.
	 *
	 * @param      Criteria $criteria
	...
	/**
	 * Method called to associate a BackendAlertI18n object to this object
	 * through the BackendAlertI18n foreign key attribute
	 *
	 * @param      BackendAlertI18n $l BackendAlertI18n
	 * @return     void
	 * @throws     PropelException
	 */
