...
/**
 * Base class that represents a row from the 'st_text' table.
 *
 * 
 *
...
abstract class BaseText extends BaseObject  implements Persistent {
	...
	/**
	 * Collection to store aggregation of collTextI18ns.
	 * @var        array
	 */
	protected $collTextI18ns;

	/**
	 * The criteria used to select the current contents of collTextI18ns.
	 * @var        Criteria
	 */
  ...
  {
    try {
      if ($this->getDispatcher()->getListeners('Text.preHydrate')) {
          $event = $this->getDispatcher()->notify(new sfEvent($this, 'Text.preHydrate', array('resultset' => $rs, 'startcol' => $startcol)));
      ...
      $this->setNew(false);
      if ($this->getDispatcher()->getListeners('Text.postHydrate')) {
          $event = $this->getDispatcher()->notify(new sfEvent($this, 'Text.postHydrate', array('resultset' => $rs, 'startcol' => $startcol + 8)));
          return $event['startcol'];
      }

      // FIXME - using NUM_COLUMNS may be clearer.
      return $startcol + 8; // 8 = TextPeer::NUM_COLUMNS - TextPeer::NUM_LAZY_LOAD_COLUMNS).
  ...
  public function delete($con = null)
  {
    ...
    if ($this->getDispatcher()->getListeners('Text.preDelete')) {
        $this->getDispatcher()->notify(new sfEvent($this, 'Text.preDelete', array('con' => $con)));
    }

    if (sfMixer::hasCallables('BaseText:delete:pre'))
    {
    ...
    if ($this->getDispatcher()->getListeners('Text.postDelete')) {
        $this->getDispatcher()->notify(new sfEvent($this, 'Text.postDelete', array('con' => $con)));
    }

    if (sfMixer::hasCallables('BaseText:delete:post'))
    {
  ...
  public function save($con = null)
  {
    ...
    if (!$this->alreadyInSave) {
      if ($this->getDispatcher()->getListeners('Text.preSave')) {
    ...
    try {
      ...
      if (!$this->alreadyInSave) {
        if ($this->getDispatcher()->getListeners('Text.postSave')) {
	...
	/**
	 * Temporary storage of collTextI18ns to save a possible db hit in
	 ...
	 * Otherwise if this Text has previously
	 * been saved, it will retrieve related TextI18ns from storage.
	 * If this Text is new, it will return
	...
	/**
	 * Returns the number of related TextI18ns.
	 *
	 * @param      Criteria $criteria
