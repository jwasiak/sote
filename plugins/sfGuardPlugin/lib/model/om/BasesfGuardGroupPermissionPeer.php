...
abstract class BasesfGuardGroupPermissionPeer {
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		if (stEventDispatcher::getInstance()->getListeners('sfGuardGroupPermissionPeer.postAddSelectColumns')) {
			stEventDispatcher::getInstance()->notify(new sfEvent($criteria, 'sfGuardGroupPermissionPeer.postAddSelectColumns'));
		}
	}
	...
	public static function doSelectJoinsfGuardGroup(Criteria $c, $con = null)
	{
		...
		$rs = sfGuardGroupPermissionPeer::doSelectRs($c, $con);
		...
		while($rs->next()) {
			...
			$obj1 = new sfGuardGroupPermission();
	...
	public static function doSelectJoinsfGuardPermission(Criteria $c, $con = null)
	{
		...
		while($rs->next()) {
			...
			$obj1 = new sfGuardGroupPermission();
			$startcol = $obj1->hydrate($rs);
                        if ($obj1->getPermissionId())
                        {

			   $obj2 = new sfGuardPermission();
			   $obj2->hydrate($rs, $startcol);
                           $obj2->addsfGuardGroupPermission($obj1);
                        }
			$results[] = self::$postHydrateMethod ? call_user_func(self::$postHydrateMethod, $obj1) : $obj1;;
