...
/**
 * Base class that represents a row from the 'st_currency_i18n' table.
 *
 * 
 *
...
abstract class BaseCurrencyI18n extends BaseObject  implements Persistent {
	...
	public function setName($v)
	{
        ...
        if ($this->name !== $v) {
          $this->name = $v;
          $this->modifiedColumns[] = CurrencyI18nPeer::NAME;
        }
  ...
  {
    try {
      if ($this->getDispatcher()->getListeners('CurrencyI18n.preHydrate')) {
          $event = $this->getDispatcher()->notify(new sfEvent($this, 'CurrencyI18n.preHydrate', array('resultset' => $rs, 'startcol' => $startcol)));
      ...
      $this->setNew(false);
      if ($this->getDispatcher()->getListeners('CurrencyI18n.postHydrate')) {
          $event = $this->getDispatcher()->notify(new sfEvent($this, 'CurrencyI18n.postHydrate', array('resultset' => $rs, 'startcol' => $startcol + 3)));
          return $event['startcol'];
      }

      // FIXME - using NUM_COLUMNS may be clearer.
      return $startcol + 3; // 3 = CurrencyI18nPeer::NUM_COLUMNS - CurrencyI18nPeer::NUM_LAZY_LOAD_COLUMNS).
  ...
  public function delete($con = null)
  {
    ...
    if ($this->getDispatcher()->getListeners('CurrencyI18n.preDelete')) {
        $this->getDispatcher()->notify(new sfEvent($this, 'CurrencyI18n.preDelete', array('con' => $con)));
    }

    if (sfMixer::hasCallables('BaseCurrencyI18n:delete:pre'))
    {
    ...
    if ($this->getDispatcher()->getListeners('CurrencyI18n.postDelete')) {
        $this->getDispatcher()->notify(new sfEvent($this, 'CurrencyI18n.postDelete', array('con' => $con)));
    }

    if (sfMixer::hasCallables('BaseCurrencyI18n:delete:post'))
    {
  ...
  public function save($con = null)
  {
    ...
    if (!$this->alreadyInSave) {
      if ($this->getDispatcher()->getListeners('CurrencyI18n.preSave')) {
          $this->getDispatcher()->notify(new sfEvent($this, 'CurrencyI18n.preSave', array('con' => $con)));
      }
    ...
    try {
      ...
      if (!$this->alreadyInSave) {
        if ($this->getDispatcher()->getListeners('CurrencyI18n.postSave')) {
            $this->getDispatcher()->notify(new sfEvent($this, 'CurrencyI18n.postSave', array('con' => $con)));
        }
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CurrencyI18nPeer::NAME)) $criteria->add(CurrencyI18nPeer::NAME, $this->name);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of CurrencyI18n (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     CurrencyI18n Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getCurrency($con = null)
	{
		if ($this->aCurrency === null && ($this->id !== null)) {
			...
			/* The following can be used instead of the line above to
			   ...
			   never be used.
			   $obj = CurrencyPeer::retrieveByPK($this->id, $con);
  ...
  {
      $event = $this->getDispatcher()->notifyUntil(new sfEvent($this, 'CurrencyI18n.' . $method, array('arguments' => $arguments, 'method' => $method)));
