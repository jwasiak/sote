 ...
 * @param object $task
 * @param array $args
 * @param string fixtures dir name example: 'fixtures', 'fixtures-data' m@sote.pl 2011-01-18
 */
function run_propel_load_data($task, $args, $fixtures_dir_name = 'fixtures')
{
  if (!count($args))
  {
    throw new Exception('You must provide the app.');
  ...
  if (count($args) == 1)
  {
    ...
    $fixtures_dirs = pakeFinder::type('dir')->ignore_version_control()->name($fixtures_dir_name)->in(array_merge($pluginDirs, array(sfConfig::get('sf_data_dir'))));
  }
  else
  {
  ...
  foreach ($fixtures_dirs as $fixtures_dir)
  {
    ...
    $data->loadData($fixtures_dir, 'propel', $fixtures_dir_name);
  }
}
 ...
 * @param string fixtures dir name example: 'fixtures', 'fixtures-data' m@sote.pl 2011-01-18
 */
function _run_propel_load_data($task, $args, $fixtures_dir_name='fixtures')
{
  if (!count($args))
  {
    throw new Exception('You must provide the app.');
