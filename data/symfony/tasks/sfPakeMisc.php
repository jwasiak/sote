 ...
 * @param array $args
 */
function run_clear_cache($task, $arg, array $options = null)
{
    ...
    $lockFile = sfConfig::get('sf_data_dir').'/.clear_cache';

    $lock = isset($options['lock']) && $options['lock'] == 'true' || !isset($options['lock']);

    if (is_file($lockFile) && time() - filemtime($lockFile) < 60)
    {
        pake_echo('clear cache is already running');
    }
    else
    {
        if ($lock)
        {
            stLock::lock('frontend', sfConfig::get('sf_root_dir'));
            pake_echo_action("Locking", "frontend");
            stLock::lock('backend', sfConfig::get('sf_root_dir'));
            pake_echo_action("Locking", "backend");
        }
        ...
        /**
         * Clear cache in smarty_c directory 
         * @author Michal Prochowski <michal.prochowski@sote.pl>
         */
        $smartyFinder = pakeFinder::type('file')->ignore_version_control()->discard('.sf');
        pake_remove($smartyFinder, sfConfig::get('sf_root_dir').DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'smarty_c');
        ...
        if ($lock)
        {        
            stLock::unlock('frontend', sfConfig::get('sf_root_dir'));
            pake_echo_action("Unlocking", "frontend");
            stLock::unlock('backend', sfConfig::get('sf_root_dir'));
            pake_echo_action("Unlocking", "backend");
        }
    }
}
