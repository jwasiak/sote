...
class sfPropelData extends sfData
{
    ...
    protected static $allFixtureFiles = null;

    protected static $loadedFixtures = array();

    protected static $objectReferences = array();

    protected function doLoadDataFromFile($fixture_file)
    {
        if (!isset(self::$loadedFixtures[$fixture_file]))
        {
            self::$loadedFixtures[$fixture_file] = true;
    ...
    public function loadData($directory_or_file = null, $connectionName = 'propel')
    {
        ...
        $this->fixture_files = $this->getFiles($directory_or_file);
        ...
        try
        {
            ...
            $this->doDeleteCurrentData($this->fixture_files);

            $this->doLoadData($this->fixture_files);
    ...
    public function loadDataFromArray($data)
    {
        ...
        foreach ($data as $class => $datas)
        {
            ...
            // create a new entry in the database
                ...
                foreach ($data as $name => $value)
                {
                    ...
                    if ($isARealColumn)
                    {
                        ...
                        if ($column->isForeignKey() && !is_null($value))
                        {
                            ...
                            if (class_exists($relatedObjectName, true))
                            {
                                $this->loadMapBuilder($relatedObjectName);
                            }

                            if ($class =='Delivery')
                            {
                                $this->findAllFixturesFiles();
                            }

                            $relatedTable = $this->maps[$class]->getDatabaseMap()->getTable($column->getRelatedTableName());

                            if (!isset(self::$objectReferences[$relatedTable->getPhpName().'_'.$value]))
                            {

                                if (!$this->doLoadFixturesFromClassName($relatedTable->getPhpName()))
                                {
                                    throw new sfException(sprintf('Failed when trying to locate the object "%s" from class "%2$s" or "%2$s.yml" fixture file', $value, $relatedTable->getPhpName()));
                                }
                            ...
                            $value = self::$objectReferences[$relatedTable->getPhpName().'_'.$value];
                        }
                    }
                ...
                {
                    self::$objectReferences[$class.'_'.$key] = $obj->getPrimaryKey();
                }
            }
        }
    ...
    protected function doLoadFixturesFromClassName($class_name)
    {
        $found = false;

        if (is_null(self::$allFixtureFiles))
        {
            $this->findAllFixturesFiles();
        }

        if (isset(self::$allFixtureFiles[$class_name]))
        {
            $this->doDeleteCurrentData(self::$allFixtureFiles[$class_name]);

            foreach (self::$allFixtureFiles[$class_name] as $fixture_file)
            {
    ...
    protected function findAllFixturesFiles()
    {

        $files = glob(sfConfig::get('sf_plugins_dir') . DIRECTORY_SEPARATOR . '*' . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'fixtures' . DIRECTORY_SEPARATOR . '*.yml');
        ...
        $files = glob(sfConfig::get('sf_data_dir') . DIRECTORY_SEPARATOR . 'fixtures' . DIRECTORY_SEPARATOR . '*.yml');
    ...
    protected function indexFixtureFile($file)
    {
        ...
        {
            if (!isset(self::$allFixtureFiles[$model_class]))
            {
                self::$allFixtureFiles[$model_class] = array();
            }
            
            self::$allFixtureFiles[$model_class][] = $file;
        }
