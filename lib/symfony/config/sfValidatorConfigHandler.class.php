...
class sfValidatorConfigHandler extends sfYamlConfigHandler
{
  ...
  protected function generateRegistration($method, &$data, &$methods, &$names, &$validators)
  {
    ...
    foreach ($methods[$method] as $name)
    {
      ...
      if (isset($attributes['hide_install_version']))
      {
        $data[] = 'if (!stSoteshop::checkInstallVersion("'.$attributes['hide_install_version'].'") {';
      }

      if (isset($attributes['old_config']))
      {
        $data[] = 'if (!stTheme::hideOldConfiguration()) {';
      }
  ...
  protected function convertAlternate2Standard(&$config)
  {
    ...
    foreach ($config['names'] as $name => $values)
    {
      ...
      {
        if (in_array($validator, array('required', 'group', 'group_msg', 'parent', 'file', 'methods', 'old_config', 'hide_install_version')))
        {
          continue;
        }
