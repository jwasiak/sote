...
class sfRouting
{
    ...
    public function generate($name, $params, $querydiv = '/', $divider = '/', $equals = '/')
    {
        ...
        else
        {
            ...
            foreach ($this->routes as $name => $route)
            {
                ...
                if (isset($defaults['module']) && $defaults['module'] != $params['module']) continue;

                if (!isset($names_hash['action']) && $defaults['action'] != $params['action']) continue;
        ...
        /**
         * Hack for PHP 5.2, 5.5
         */ 
        $this->realUrlParams = $params;
        $real_url = preg_replace_callback('/\:([^\/]+)/', array($this, 'replace_real_url'), $url);
        unset($this->realUrlParams);
    ...
    protected function replace_real_url($m) {
        return urldecode($this->realUrlParams[$m[1]]);
    }
}
