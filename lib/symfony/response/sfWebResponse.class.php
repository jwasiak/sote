...
class sfWebResponse extends sfResponse
{
  ...
  {
    return preg_replace_callback('/\-(.)/', create_function('$m', 'return \'-\'.strtoupper($m[1]);'), strtr(ucfirst(strtolower($name)), '_', '-'));
  ...
  {
    return $this->getParameterHolder()->getAll('helper/asset/auto/link');
  }

  public function addLink($href, $rel, $options = array())
  {
    $options['rel'] = $rel;
    $options['href'] = $href;

    $this->setParameter($href, $options, 'helper/asset/auto/link');
  }

  public function setCanonicalLink($href)
  {
    $this->setParameter('canonical', array('rel' => 'canonical', 'href' => $href), 'helper/asset/auto/link');
  }
