...
if (!sfConfig::get('sf_in_bootstrap'))
{
  ...
  require_once(sfConfig::get('sf_lib_dir') . DIRECTORY_SEPARATOR . 'symfony' . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . 'sfLoader.class.php');
  require_once(sfConfig::get('sf_lib_dir') . DIRECTORY_SEPARATOR . 'symfony' . DIRECTORY_SEPARATOR . 'controller' . DIRECTORY_SEPARATOR . 'sfRouting.class.php');
...
try
{
  $sf_in_bootstrap = sfConfig::get('sf_in_bootstrap');
  ...
  if (!$sf_in_bootstrap && sfConfig::get('sf_logging_enabled', true))
  {
    include($configCache->checkConfig($sf_app_config_dir_name.'/logging.yml'));
  }
  if (!$sf_in_bootstrap && ($file = $configCache->checkConfig($sf_app_config_dir_name.'/app.yml', true)))
  {
    include($file);
  }
  if (!$sf_in_bootstrap && sfConfig::get('sf_i18n'))
  {
  ...
  $configCache->import($sf_app_config_dir_name.'/config_compile.yml', false);
