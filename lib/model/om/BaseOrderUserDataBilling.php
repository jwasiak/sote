...
/**
 * Base class that represents a row from the 'st_order_user_data_billing' table.
 *
 * 
 *
...
abstract class BaseOrderUserDataBilling extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the has_valid_vat_eu field.
	 * @var        boolean
	 */
	protected $has_valid_vat_eu = false;
    ...
    /**
     * Get the [has_valid_vat_eu] column value.
     * 
     * @return     boolean
     */
	...
	/**
	 * Set the value of [has_valid_vat_eu] column.
	 * 
	 * @param      boolean $v new value
	 * @return     void
	 */
	public function setHasValidVatEu($v)
	{
        ...
        if ($this->has_valid_vat_eu !== $v || $v === false) {
          $this->has_valid_vat_eu = $v;
          $this->modifiedColumns[] = OrderUserDataBillingPeer::HAS_VALID_VAT_EU;
        }
	...
	public function setPesel($v)
	{
        ...
        if ($this->pesel !== $v) {
          $this->pesel = $v;
          $this->modifiedColumns[] = OrderUserDataBillingPeer::PESEL;
        }
  ...
  {
    try {
      if ($this->getDispatcher()->getListeners('OrderUserDataBilling.preHydrate')) {
          $event = $this->getDispatcher()->notify(new sfEvent($this, 'OrderUserDataBilling.preHydrate', array('resultset' => $rs, 'startcol' => $startcol)));
      ...
      $this->has_valid_vat_eu = $rs->getBoolean($startcol + 4);

      $this->country = $rs->getString($startcol + 5);

      $this->full_name = $rs->getString($startcol + 6);
      ...
      $this->surname = $rs->getString($startcol + 11);

      $this->street = $rs->getString($startcol + 12);

      $this->house = $rs->getString($startcol + 13);

      $this->flat = $rs->getString($startcol + 14);

      $this->code = $rs->getString($startcol + 15);

      $this->town = $rs->getString($startcol + 16);

      $this->phone = $rs->getString($startcol + 17);

      $this->company = $rs->getString($startcol + 18);

      $this->vat_number = $rs->getString($startcol + 19);

      $this->pesel = $rs->getString($startcol + 20);
      ...
      $this->setNew(false);
      if ($this->getDispatcher()->getListeners('OrderUserDataBilling.postHydrate')) {
          $event = $this->getDispatcher()->notify(new sfEvent($this, 'OrderUserDataBilling.postHydrate', array('resultset' => $rs, 'startcol' => $startcol + 22)));
          return $event['startcol'];
      }

      // FIXME - using NUM_COLUMNS may be clearer.
      return $startcol + 22; // 22 = OrderUserDataBillingPeer::NUM_COLUMNS - OrderUserDataBillingPeer::NUM_LAZY_LOAD_COLUMNS).
  ...
  public function delete($con = null)
  {
    ...
    if ($this->getDispatcher()->getListeners('OrderUserDataBilling.preDelete')) {
        $this->getDispatcher()->notify(new sfEvent($this, 'OrderUserDataBilling.preDelete', array('con' => $con)));
    }

    if (sfMixer::hasCallables('BaseOrderUserDataBilling:delete:pre'))
    {
    ...
    if ($this->getDispatcher()->getListeners('OrderUserDataBilling.postDelete')) {
        $this->getDispatcher()->notify(new sfEvent($this, 'OrderUserDataBilling.postDelete', array('con' => $con)));
    }

    if (sfMixer::hasCallables('BaseOrderUserDataBilling:delete:post'))
    {
  ...
  public function save($con = null)
  {
    ...
    if (!$this->alreadyInSave) {
      if ($this->getDispatcher()->getListeners('OrderUserDataBilling.preSave')) {
    ...
    try {
      ...
      if (!$this->alreadyInSave) {
        if ($this->getDispatcher()->getListeners('OrderUserDataBilling.postSave')) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[13] => $this->getHouse(),
			$keys[14] => $this->getFlat(),
			$keys[15] => $this->getCode(),
			$keys[16] => $this->getTown(),
			$keys[17] => $this->getPhone(),
			$keys[18] => $this->getCompany(),
			$keys[19] => $this->getVatNumber(),
			$keys[20] => $this->getPesel(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setHasValidVatEu($arr[$keys[4]]);
		...
		if (array_key_exists($keys[13], $arr)) $this->setHouse($arr[$keys[13]]);
		if (array_key_exists($keys[14], $arr)) $this->setFlat($arr[$keys[14]]);
		if (array_key_exists($keys[15], $arr)) $this->setCode($arr[$keys[15]]);
		if (array_key_exists($keys[16], $arr)) $this->setTown($arr[$keys[16]]);
		if (array_key_exists($keys[17], $arr)) $this->setPhone($arr[$keys[17]]);
		if (array_key_exists($keys[18], $arr)) $this->setCompany($arr[$keys[18]]);
		if (array_key_exists($keys[19], $arr)) $this->setVatNumber($arr[$keys[19]]);
		if (array_key_exists($keys[20], $arr)) $this->setPesel($arr[$keys[20]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(OrderUserDataBillingPeer::HAS_VALID_VAT_EU)) $criteria->add(OrderUserDataBillingPeer::HAS_VALID_VAT_EU, $this->has_valid_vat_eu);
		...
		if ($this->isColumnModified(OrderUserDataBillingPeer::PESEL)) $criteria->add(OrderUserDataBillingPeer::PESEL, $this->pesel);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of OrderUserDataBilling (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setHasValidVatEu($this->has_valid_vat_eu);
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     OrderUserDataBilling Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     OrderUserDataBillingPeer
	 */
	 ...
	 * Otherwise if this OrderUserDataBilling has previously
	 * been saved, it will retrieve related Orders from storage.
	 * If this OrderUserDataBilling is new, it will return
	 ...
	 * If this collection has already been initialized with
	 * an identical criteria, it returns the collection.
	 * Otherwise if this OrderUserDataBilling is new, it will return
	 * an empty collection; or if this OrderUserDataBilling has previously
	 * been saved, it will retrieve related Orders from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in OrderUserDataBilling.
	 */
