...
$smarty->assign("show_name", $config->get('show_name_short'));

$smarty->assign("show_image", $config->get('show_image_short'));

$smarty->assign("show_code", $config->get('show_code_short'));

$smarty->assign("show_price", $config->get('show_price_short'));

$smarty->assign("show_old_price", $config->get('show_old_price_short'));

$smarty->assign("show_discount", $config->get('show_discount_short'));

$smarty->assign("show_basket", $config->get('show_basket_short'));

$smarty->assign("price_view", $config->get('price_view_short'));

$smarty->assign('show_weight', $config->get('show_weight_short'));

$photo_max_height = st_asset_thumbnail_setting('height', 'icon');

$cut_name = $config->get('cut_name_short');

$max_name_length = $config->get('cut_name_num_short');
...
$smarty->assign('show_points', $config_points->get('product_short_list_show_points'));
$smarty->assign('display_type', $config_points->get('product_short_list_display_type'));
...
foreach ($product_pager->getResults() as $index => $product)
{
    ...
    $results[$index]['photo'] = content_tag('a', st_product_image_tag($product, 'icon'), array('href' => $product_url));
    ...
    {
        if ($config->get('show_uom_short') && $product->getUom())
        {
            $uom = " / ".$product->getUom();
        }else{
            $uom = "";
        ...
        $results[$index]['basket'] = st_get_component('stBasket', 'add', array('product' => $product, 'simple' => true));

        $results[$index]['discount'] = $product->getDiscountInPercent();

        $results[$index]['check_price'] = false;

        if ($config->get('show_basic_price_short') && $product->hasBasicPrice() && $product->getBasicPriceBrutto()!=0)
        {
...
$smarty->assign('show_basic_price_short', $config->get('show_basic_price_short'));

$smarty->display('product_list_short.html');
?>